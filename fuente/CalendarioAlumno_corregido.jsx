...
import { Box, Typography, Grid, Autocomplete, TextField, Button, Paper, CircularProgress, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Checkbox, InputAdornment, IconButton, Modal, Fade, Backdrop, List, ListItem, ListItemText, Card, CardContent, Chip } from '@mui/material';
...
const CalendarioAlumno = ({ darkMode, setDarkMode }) => {
  ...
  const [cursos, setCursos] = useState([]); // [{codigo: 'MAT101', nombre: 'Matemática I'}]
  const [cursosSeleccionados, setCursosSeleccionados] = useState([]); // [{codigo, nombre}]
  ...
  const handleMostrarHorario = async () => {
    ...
    const horarioActual = horariosGuardados[0]; // Asumiendo que guardamos solo uno por ahora
    if (!horarioActual.cursos || !horarioActual.cursos.length) return;
    
    try {
      // Consulta al backend para todos los cursos guardados con probabilidades
      const codigos = horarioActual.cursos.map(c => c.codigo);
      const res = await axios.post('/api/horario-cursos', {
        codigos,
        periodo: horarioActual.ciclo || ciclo,
        nota_alumno: parseFloat(nota) // Enviar la nota del alumno
      });
      
      // Procesar la respuesta para el componente HorarioSemanal
      const secciones = res.data.secciones || [];
      const bloquesProcesados = [];
      
      // Convertir secciones a formato de bloques para HorarioSemanal
      secciones.forEach(seccion => {
        seccion.horarios.forEach(horario => {
          bloquesProcesados.push({
            ...
            resultado_probabilidad: seccion.resultado_probabilidad,
            color_probabilidad: seccion.color_probabilidad,
            turno_estimado: seccion.turno_estimado,
            profesor_demanda: seccion.profesor_demanda,
            cruces: seccion.cruces
          });
        });
      });
  ...
  // Nueva función para optimizar horario
  const handleOptimizarHorario = async () => {
    if (!cursosSeleccionados.length || !nota) {
      alert('Debe seleccionar cursos y ingresar su nota para optimizar el horario');
      return;
    }
    ...
    try {
      const codigos = cursosSeleccionados.map(c => c.codigo);
      ...
      if (res.data.success) {
        setHorarioOptimizado(res.data.horario_optimizado);
        ...
        // Convertir horario optimizado a formato de bloques
        const bloquesOptimizados = [];
        res.data.horario_optimizado.forEach(seccion => {
          seccion.horarios.forEach(horario => {
            bloquesOptimizados.push({
              codigo_curso: seccion.codigo_curso,
              nombre_curso: cursosSeleccionados.find(c => c.codigo === seccion.codigo_curso)?.nombre || seccion.codigo_curso,
              seccion: seccion.seccion,
              profesor: seccion.profesor,
              dia: horario.dia,
              hora_inicio: horario.hora_inicio,
        ...
        setBloquesHorario(bloquesOptimizados);
        setMostrarHorario(true);
      }
    } catch (error) {
      setMostrarHorario(false);
        ...
        <Box sx={{ width: '100%', maxWidth: 1200, mt: 4 }}>
                      ...
                      {mensajeGuardado.startsWith('¡') && cursosGuardados.length > 0 && (
                        <Box sx={{ display: 'flex', gap: 2, mt: 2, flexWrap: 'wrap', justifyContent: 'center' }}>
                          ...
                          <Button
                            variant="contained"
                            color="warning"
                            onClick={handleOptimizarHorario}
                            disabled={loadingOptimizacion}
                            startIcon={loadingOptimizacion ? <CircularProgress size={20} color="inherit" /> : null}
                            sx={{
                              fontWeight: 'bold',
                              fontSize: 16,
                              borderRadius: 3,
                              px: 3,
                              py: 1,
                              background: darkMode ? '#ff9800' : '#ff9800',
                              color: '#ffffff',
                              transition: 'all 0.3s ease',
                              '&:hover': {
                                background: darkMode ? '#f57c00' : '#f57c00',
                                transform: 'scale(1.02)',
                          ...
                          >
                            {loadingOptimizacion ? 'Optimizando...' : 'Optimizar Horario'}
                          </Button>
                        </Box>
                      )}
                    </>
          ...
          {mostrarHorario && (
            ...
            >
              ...
              >
                {mostrarHorarioOptimizado ? 'Horario Optimizado' : 'Tu Horario Semanal'}
              </Typography>
              ...
              {(informacionHorario.resumen || mostrarHorarioOptimizado) && (
                <Box sx={{ mb: 3, p: 3, borderRadius: 3, background: darkMode ? '#2d2d2d' : '#f8f9fa' }}>
                  ...
                  >
                    {mostrarHorarioOptimizado ? 'Horario Optimizado' : 'Resumen de Análisis'}
                  </Typography>
                    ...
                    {!mostrarHorarioOptimizado && (
                      <>
                        <Grid item xs={12} sm={6} md={3}>
                          <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, background: darkMode ? '#1e1e1e' : '#ffffff' }}>
                            <Typography variant="h4" sx={{ color: '#4caf50', fontWeight: 'bold' }}>
                              {informacionHorario.resumen?.secciones_sin_cruces_criticos || 0}
                            </Typography>
                            <Typography variant="body2" sx={{ color: darkMode ? '#90caf9' : '#546e7a' }}>
                              Secciones Sin Cruces Críticos
                            </Typography>
                          </Box>
                        </Grid>
                        ...
                        <Grid item xs={12} sm={6} md={3}>
                          <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, background: darkMode ? '#1e1e1e' : '#ffffff' }}>
                            <Typography variant="h4" sx={{ color: '#f44336', fontWeight: 'bold' }}>
                              {informacionHorario.resumen?.secciones_con_cruces_criticos || 0}
                            </Typography>
                            <Typography variant="body2" sx={{ color: darkMode ? '#90caf9' : '#546e7a' }}>
                              Secciones Con Cruces Críticos
                            </Typography>
                          </Box>
                        </Grid>
                        ...
                        <Grid item xs={12} sm={6} md={3}>
                          <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, background: darkMode ? '#1e1e1e' : '#ffffff' }}>
                            <Typography variant="h4" sx={{ color: '#1976d2', fontWeight: 'bold' }}>
                              {informacionHorario.resumen?.total_secciones || 0}
                            </Typography>
                            <Typography variant="body2" sx={{ color: darkMode ? '#90caf9' : '#546e7a' }}>
                              Total de Secciones
                            </Typography>
